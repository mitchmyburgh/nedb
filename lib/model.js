...
/**
 * Removes all instances of an array from an existing array
 */
lastStepModifierFunctions.$pullAll = (obj, field, values) => {
  if (!Array.isArray(obj[field])) throw new Error('Can\'t $pullArray an element from non-array values')

  const arr = obj[field]
  for (let i = arr.length - 1; i >= 0; i -= 1) {
    for (let j = 0; j < values.length; j += 1) {
      if (match(arr[i], values[j])) arr.splice(i, 1)
    }
  }
}
...
/*
{
  '$pullAll': { strings: [ 'foo', 'baz', 'bat' ] },
  '$set': { _updated_at: 2021-11-25T09:29:34.153Z }
}
